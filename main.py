#the bot was made in diskord.py 1.7.3		бот был сделан на diskord.py 1.7.3
import CONFIG
import discord
					...
					if member.activity is not None:
						if member.activity.name is not None:
							for i in range(len(CONFIG.BANNED_GAMES)):
								if member.activity.name.lower() == CONFIG.BANNED_GAMES[i].lower():
									if (datetime.utcnow() - member.activity.start).total_seconds() > CONFIG.BAN_TIME:
